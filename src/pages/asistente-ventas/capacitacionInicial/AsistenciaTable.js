...
const CustomRow = ({ item }) => {
  ...
  const jefeVenta = useJefesVentas().find((j) => j.id === jefeVentaId);
  ...
  // console.log('jefeVenta', jefeVenta, jefeVenta?.empleado_id);
  // console.log('tipoCuentaJefeVentas', tipoCuentaJefeVentas);
  // console.log('tipoCuentaJefeVentasId', tipoCuentaJefeVentasId);
  ...
  return (
    <TableContainer sx={{ width: '100%', overflowX: 'auto' }} style={{ width: '100%' }} component={Paper}>
      <Table sx={{ minWidth: 650 }} aria-label="simple table">
        ...
        <TableBody>
          {rows.length === 0 ? (<EmptyTable msg="No existen datos a mostrar" colSpan={10} />) : (rows.map((item) => <CustomRow key={item.id} item={item} />))}
        </TableBody>
      </Table>
    </TableContainer>
  );
