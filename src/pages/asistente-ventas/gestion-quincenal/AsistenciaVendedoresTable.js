...
const CustomTable = ({ data, sendEmail, handleDelete, handleAsistencias, handleUpdate }) => {
  ...
  const handleEliminar = async (id) => {
    ...
    try {
      ...
      dispatch(deleteReunion({ id: data.id, tipo_reunion: REUNION.Quincenal }));
  ...
  const handleUpdateAsistencias = async (id) => {
    ...
    try {
      ...
      dispatch(setReunionSent({ reunion_id: id, tipo_reunion: REUNION.Quincenal }));
  ...
  const sendMail = async (id) => {
    ...
    try {
      ...
      await sendEmail(id);
      dispatch(setMailSent({ reunion_id: id, tipo_reunion: REUNION.Quincenal }));
            ...
            <TableRow sx={{ backgroundColor: '#d7d7d7' }}>
              ...
              <TableCell align="center">
                <div style={{ display: 'inline-block', paddingLeft: 3, paddingRight: 3 }}>
                  <EditarModalCQV
                    handleEditarCapacitacion={handleUpdate}
                    capacitacion={data}
                  />
                </div>
                ...
                <div style={{ display: 'inline-block', paddingLeft: 3, paddingRight: 3 }}>
                  <DeleteModalCQV
                    handleDeleteCapacitacion={handleEliminar}
                    capacitacion={data}
                  />
                </div>
              </TableCell>
...
const AsistenciaVendedoresTable = ({ handleDelete, handleAsistencias, handleUpdate, sendEmail }) => {
  ...
  const data = useSelector(state => state.reunionGeneral[REUNION.Quincenal].reuniones);
  ...
  useEffect(() => {
    ...
    const rows = data.filter(d => !d.completada).map((c, index) => {
      ...
      return {
        ...
        completada: c.completada, // Para usarlo localmente
        activa_enviar
      };
      ...
      {rows.length === 0 ? (
        <TableContainer component={Paper}>
          <Table sx={{ minWidth: 650, backgroundColor: 'white', border: '2px solid lightblue' }} aria-label="simple table">
            ...
            <TableBody>
              <EmptyTable msg="No existen capacitaciones creadas" colSpan={5} />
            </TableBody>
          </Table>
        </TableContainer>
      ) : (
