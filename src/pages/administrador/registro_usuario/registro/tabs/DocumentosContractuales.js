...
const DocumentosContractuales = () => {
  ...
  const handleSubmit = async (e, tipo) => {
    ...
    try {
      ...
      if (copiaCedula.file && !hojaDeVida.file) throw new Error('Archivo no seleccionado');
      if (copiaCedula.change && !hojaDeVida.change) throw new Error('Escoger otro documento para subir');
  ...
  const handleAceptar = async (id, documento) => {
    ...
    try {
      ...
      console.error('Error al enviar accepation de documento', error);
