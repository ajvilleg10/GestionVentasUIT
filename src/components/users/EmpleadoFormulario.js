...
const EmpleadoFormulario = ({ formik, tipos }) => {
  ...
  const [selectAsistente, setSelectAsistente] = useState(false);
      ...
      <List>
              ...
              <FormControl fullWidth>
                <Select
                  ...
                  onChange={(e) => {
                    ...
                    if (tipo.alias === 'jefe_ventas') setSelectAsistente(true);
                    else setSelectAsistente(false);

                    if (tipo.alias === 'director_operaciones') setDirectorSelected(true);
                    else setDirectorSelected(false);

                    if (tipo.alias.includes('vendedor')) setVendedorSelected(true);
        ...
        {selectAsistente && (
          <ListItem >
            <Grid container spacing={3}>
              <Grid item xs={12} md={2} display="flex" alignItems="center">
                <InputLabel id="asistente_ventas_id">Asistente Ventas*</InputLabel>
              </Grid>
              <Grid item xs={12} md={6}>
                <FormControl fullWidth>
                  <Select
