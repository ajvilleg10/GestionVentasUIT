...
const ReferidosModal = ({ handleAcceptance, row, disabled }) => {
  ...
  const transformarNombres = (e) => {
    referidos.map((row) => {
      if (row["nombre"] === '' || row["parentezco"] === '' || row["numero"] === '')
        throw new Error('Llene todos los campos');
      const nombre = row["nombre"].split(" ");
      if (nombre.length !== 3)
        throw new Error('Ingrese un nombre y dos Apellidos');
      row["primerNombre"] = nombre[0];
      row["apellido"] = nombre[1] + " " + nombre[2];
