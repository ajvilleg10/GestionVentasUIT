...
import { DIAS, VIERNES, MODALIDADES, REUNION } from 'utils/constants';
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        if (multiple) {
          ...
          const dates = getDates(values.fecha_reunion_incio_batch, values.fecha_reunion_final_batch, values.date, REUNION.Mensual);
          ...
          const rs = await crearReunion(newCMVs);

          snackbar.message = rs.message ?? 'Comités mensuales creados con éxito';
          ...
          const rs = await crearReunion(newCMV);

          snackbar.message = rs.message ?? 'Comité semanal creado con éxito';
          snackbar.alert.color = 'success';
        ...
        snackbar.message = error.message ?? 'Error al crear el comité mensual';
