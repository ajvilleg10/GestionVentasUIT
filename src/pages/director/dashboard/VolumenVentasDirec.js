...
function obtenerDatosPorFechaCards(year, mes, idJefeVentas, idJefeSucursal) {
...
function obtenerDataGraficoVentas(dataGraficaVentas, vendedorId) {
  ...
  if (vendedorId !== "Todos") {
    const selectedVendedor = dataGraficaVentas.find(
    ...
    dataGraficaVentas.forEach((vendedor) => {
      vendedor.dataGrafica.forEach((mes, index) => {
        dataTotales[index].data = dataTotales[index].data.map(
          (valor, i) => valor + mes.data[i]
        );
      });
    });
...
function obtenerDataGraficoContratosInactivos(
  dataGraficaContratosInactivosIni,
  vendedorId
) {
  if (
    !dataGraficaContratosInactivosIni ||
    dataGraficaContratosInactivosIni.length === 0
  ) {
    return {};
  }
  if (vendedorId !== "Todos") {
    const selectedVendedor = dataGraficaContratosInactivosIni.find(
      (vend) => vend.id === vendedorId
    );
    return selectedVendedor.dataGrafica;
  } else {
...
function obtenerDataGraficoVentasPorOrigen(
  dataGraficaVentasXOrigen,
  vendedorId
) {
  ...
  if (vendedorId !== "Todos") {
    const selectedVendedor = dataGraficaVentasXOrigen.find(
      (vend) => vend.id === vendedorId
    );
    return selectedVendedor.dataGrafica;
  } else {
...
function obtenerDataGraficoVendedores(dataGraficaVendedores, vendedorId) {
  ...
  if (vendedorId !== "Todos") {
    const selectedVendedor = dataGraficaVendedores.find(
      (vend) => vend.id === vendedorId
    );
    return selectedVendedor.dataGrafica;
  } else {
    ...
    dataVendedores.forEach((vendedor) => {
      Object.keys(vendedor.dataCards).forEach((categoria) => {
        if (!["sin_titulo", "bonificacion"].includes(categoria)) {
          if (
            [
              ...
              "vendedores_entrantes",
              "vendedores_salientes",
            ].includes(categoria)
          ) {
        ...
        if (
          ...
          datosTotales[categoria].contratos +=
            vendedor.dataCards[categoria].contratos;
        }

        if (
          ["vendedores_finales", "sin_titulo", "cotratos_por_renovar"].includes(
            categoria
          )
        ) {
...
let dataGraficaVendSelec = obtenerDataGraficoVendedores(
  dataGraficaVendedoresIni,
  "Todos"
);
let dataCardsIni = obtenerDatosCards(dataVendedoresIni, "Todos");
let dataGraficaVentaXOrigenSelec = obtenerDataGraficoVentasPorOrigen(
  dataGraficaXOrigenIni,
  "Todos"
);

let dataGraficaContratosInactivosSelecIni =
  obtenerDataGraficoContratosInactivos(
    dataGraficaContratosInactivosIni,
    "Todos"
  );

let dataGraficaVentasSelecIni = obtenerDataGraficoVentas(
  dataGraficaVentasIni,
  "Todos"
);

const jefesSucursal = [
  { id: 1, nombre: "Jefe Sucursal 1" },
  { id: 2, nombre: "Jefe Sucursal 2" },
  // Agrega más jefes de sucursal según sea necesario
];

const jefesVenta = [
  { id: 1, nombre: "Jefe Venta 1" },
  { id: 2, nombre: "Jefe Venta 2" },
  // Agrega más jefes de venta según sea necesario
];

function VolumenVentasDirec() {
  const [dataVendedoresState, setDataVendedores] = useState(dataVendedoresIni);
  const [dataGraficaVendedoresState, setDataGraficaVendedores] = useState(
    dataGraficaVendedoresIni
  );
  const [dataGraficaVendedorSelecState, setDataGraficaVendedoresSelec] =
    useState(dataGraficaVendSelec);

  const [dataCards, setDataCards] = useState(dataCardsIni);
  const [dataGraficaVentasXOrigenState, setDataGraficaVentasXOrigen] = useState(
    dataGraficaXOrigenIni
  );
  const [dataGraficaVentasXOrgienSelec, setDataGraficaVentasXOrigenSelec] =
    useState(dataGraficaVentaXOrigenSelec);

  const [dataGraficaContratosInactivosState, setDataGraficaContratosInactivos] =
    useState(dataGraficaContratosInactivosIni);
  const [
    dataGraficaContratosInactivosSelec,
    setDataGraficaContratosInactivosSelec,
  ] = useState(dataGraficaContratosInactivosSelecIni);

  const [dataGraficaVentasState, setDataGraficaVentas] =
    useState(dataGraficaVentasIni);
  const [dataGraficaVentasSelec, setDataGraficaVentasSelec] = useState(
    dataGraficaVentasSelecIni
  );

  const [dataGraficaSubtipoPlanState, setDataGraficaSubtipoPlan] = useState(
    dataGraficaSubtipoPlanIni
  );
  ...
  // Estado para almacenar los jefes de sucursal y jefes de venta seleccionados
  const [jefeSucursalSeleccionado, setJefeSucursalSeleccionado] =
    useState("Todos");
  const [jefeVentaSeleccionado, setJefeVentaSeleccionado] = useState("Todos");

  // Manejador de cambio para el jefe de sucursal
  const handleJefeSucursalChange = (event) => {
    const selectedJefeSucursal = event.target.value;
    setJefeSucursalSeleccionado(selectedJefeSucursal);

    // Aquí puedes realizar las operaciones necesarias con el jefe de sucursal seleccionado
    // Por ejemplo, puedes filtrar los datos según el jefe de sucursal seleccionado
  };

  // Manejador de cambio para el jefe de venta
  const handleJefeVentaChange = (event) => {
    const selectedJefeVenta = event.target.value;
    setJefeVentaSeleccionado(selectedJefeVenta);

    // Aquí puedes realizar las operaciones necesarias con el jefe de venta seleccionado
    // Por ejemplo, puedes filtrar los datos según el jefe de venta seleccionado
  };
  ...
  const handleVendedorChange = (event) => {
    ...
    if (event.target.value !== "Todos") {
      ...
      setDataGraficaVendedoresSelec(
        obtenerDataGraficoVendedores(
          dataGraficaVendedoresState,
          event.target.value
        )
      );
      setDataGraficaVentasXOrigenSelec(
        obtenerDataGraficoVentasPorOrigen(
          dataGraficaVentasXOrigenState,
          event.target.value
        )
      );
      setDataGraficaContratosInactivosSelec(
        obtenerDataGraficoContratosInactivos(
          dataGraficaContratosInactivosState,
          event.target.value
        )
      );
      setDataGraficaVentasSelec(
        obtenerDataGraficoVentas(dataGraficaVentasState, event.target.value)
      );
    } else {
      setDataCards(obtenerDatosCards(dataVendedoresState, "Todos"));
      setDataGraficaVendedoresSelec(
        obtenerDataGraficoVendedores(dataGraficaVendedoresState, "Todos")
      );
      setDataGraficaVentasXOrigenSelec(
        obtenerDataGraficoVentasPorOrigen(
          dataGraficaVentasXOrigenState,
      ...
      setDataGraficaVentasSelec(
        obtenerDataGraficoVentas(dataGraficaVentasState, "Todos")
      );
    }
  };
                    ...
                    <Grid container justifyContent="space-between">
                      <Box sx={{ width: "150px" }}>
                        <FormControl fullWidth>
                          <InputLabel id="jefeSucursalLabel">
                            Seleccione el Jefe de Sucursal
                          </InputLabel>
                          <Select
                            labelId="jefeSucursalLabel"
                            id="jefeSucursalSelect"
                            value={jefeSucursalSeleccionado}
                            onChange={handleJefeSucursalChange}
                          >
                            <MenuItem value="Todos">Todos</MenuItem>
                            {jefesSucursal.map((jefe) => (
                      ...
                      <Box sx={{ width: "150px" }}>
                        <FormControl fullWidth>
                          <InputLabel id="jefeSucursalLabel">
                            Seleccione el Jefe de Venta
                          </InputLabel>
                          <Select
                            labelId="jefeVentaLabel"
                            id="jefeVentaSelect"
                            value={jefeVentaSeleccionado}
                            onChange={handleJefeVentaChange}
                          >
                            <MenuItem value="Todos">Todos</MenuItem>
                            {jefesVenta.map((jefe) => (
                              <MenuItem key={jefe.id} value={jefe.id}>
                ...
                >
                  <Table>
                    <TableBody>
                      <TableRow>
                        <GraficoVendedores
                          data={dataGraficaVendedorSelecState}
                        />
                  ...
                  <Table>
                    <TableBody>
                      <TableRow>
                        <GraficoVentasPorOrigen
                          data={dataGraficaVentasXOrgienSelec}
                        />
                      ...
                      <TableRow>
                        <TableCell style={{ border: "40px solid #c1e6df" }}>
                          <div>
                            ...
                            <GraficoContratosInactivos
                              data={dataGraficaContratosInactivosSelec}
                            />
                          </div>
                        </TableCell>
                      </TableRow>
...
export default VolumenVentasDirec;
