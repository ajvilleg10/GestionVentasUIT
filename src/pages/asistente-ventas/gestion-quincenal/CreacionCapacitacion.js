...
import { DIAS, REUNION, VIERNES, MODALIDADES } from 'utils/constants';
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        if (multiple) {
          ...
          const dates = getDates(values.fecha_reunion_incio_batch, values.fecha_reunion_final_batch, values.date, REUNION.Quincenal);
          ...
          const CQVs = dates.map((fecha) => {
            ...
            return {
              ...
              tipo_reunion: REUNION.Quincenal
            };
          });
          ...
          const rs = await crearCapacitacion(newCQVs);

          snackbar.message = rs.message ?? 'Capacitaciones quincenales creadas con éxito';
          ...
          const rs = await crearCapacitacion(newCQV);

          snackbar.message = rs.message ?? 'Capacitación quincenal creada con éxito';
          snackbar.alert.color = 'success';
