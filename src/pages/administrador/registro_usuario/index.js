...
const EmpleadoView = () => {
  ...
  useEffect(() => {
    const rows = usuarios.slice(0, rowsPerPage);
    setPageRows(rows);
  }, [usuarios]);
          ...
          <Grid item width={'100%'}>
            <TableContainer sx={{ width: '100%', overflowX: 'auto' }} style={{ width: '100%' }} component={Paper}>
              <Table sx={{ width: '100%', backgroundColor: 'white', border: '2px solid lightblue' }} aria-label="simple table">
                ...
                <TableBody>
                  {pageRows.length === 0 ? (<EmptyTable msg="No existen usuarios" colSpan={CREACION_USUARIOS_HEADERS.length} />) : (
                    pageRows.map((u, i) => (
                      <TableRow
                        hover
                        role="checkbox"
                  ...
                  {usuarios.length > 0 && (
                    <TableRow>
                      <TableCell colSpan={CREACION_USUARIOS_HEADERS.length} sx={{ p: 2 }}>
                        <Paginacion
                          data={usuarios}
