...
const validationSchema = yup.object({
  nombres: yup.string().matches(/^[^\d]+$/, "Nombre incorrecto").required('Campo requerido'),
  apellidos: yup.string().matches(/^[^\d]+$/, "Apellido incorrecto").required('Campo requerido'),
  numero_celular: yup.string().required('Campo requerido').test('phone_number', 'Número de celular inválido', (value) => {
    if (value === undefined) return true;
    return value.match(/^09\d{8}$/);
  }),
  comentarios: yup.string().required('Campo requerido'),
  parentezco: yup.string().matches(/^[^\d]+$/, "Parentesco incorrecto").required('Campo requerido')
});
            ...
            <Paper sx={{ p: 2 }}>
              ...
              <Grid container spacing={0}>
                ...
                <Grid item md={2} xs={2} display="flex" justifyContent="end">
                  <Button variant="contained" color="primary" type="submit" disabled={isFormDisabled} style={{ maxHeight: '40px' }}>
                    Agregar
                  </Button>
                </Grid>
              </Grid>
        ...
        <Box mt={2}>
          <TableContainer sx={{ width: '100%', overflowX: 'auto' }} style={{ width: '100%' }}>
            <Table>
              ...
              <TableBody>
                {contactos.length === 0 ? (
                  <EmptyTable msg="No existen contactos" colSpan={headCells.length} />
                ) : (
          ...
          <Grid container spacing={0} py={3}>
            <Grid item md={12} xs={12} display="flex" justifyContent="right" alignItems="center">
              {send ? <ContactosEnviadosDialog onOpenDialog={enviarContactosHandler} isProyecto100Enviado={isProyecto100Enviado} /> : ''}
            </Grid>
          </Grid>
        </Box>
      </Box>
