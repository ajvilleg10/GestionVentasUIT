...
import { setId } from 'store/reducers/contactoSeleccionado';
...
const ContactosTable = ({ data, showDetails, createFunction }) => {
  ...
  useEffect(() => {
    const rows_slice = data?.slice(0, rowsPerPage);
    setPageRows(rows_slice);
    // console.log(data);
    // setRows(data);
  }, [data]);
        ...
        <Table sx={{ minWidth: 650, backgroundColor: 'white', border: '2px solid lightblue' }} aria-label="simple table">
          <TableHead >
            <TableRow sx={{ backgroundColor: '#d7d7d7' }}>
              <TableCell sx={{ minWidth: '170px' }} align="left">Nombres</TableCell>
              <TableCell sx={{ minWidth: '170px' }} align="left">Apellidos</TableCell>
              <TableCell sx={{ minWidth: '170px' }} align="left">Celular</TableCell>
              <TableCell sx={{ minWidth: '170px' }} align="left">Comentario</TableCell>
              <TableCell sx={{ minWidth: '170px' }} align="left">Origen de Contacto</TableCell>
              <TableCell sx={{ minWidth: '170px' }} align="left">Estado</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            <TableRow>
              ...
              <TableCell align="left" size="small">
                <Select name={'origen'} onChange={handleChange} fullWidth>
                  {origenes?.map((origen) => (
