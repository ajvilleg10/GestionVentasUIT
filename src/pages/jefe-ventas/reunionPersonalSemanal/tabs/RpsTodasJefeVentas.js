...
const RpsTodasJefeVentas
  ...
  = () => {
    ...
    const reunionesOfCurrentUser = reuniones.filter((reunion) => {
      ...
      return reunion.empleados.some((empleado) => empleado.empleadoId === currentUserEmpleadoId) && reunion.enviado === true;
    });
    ...
    const reunionTable = reunionesSorted?.filter((reunion) => {
      ...
      return (
        <Grid item xs={12} md={6}>
          <Link style={{ textDecoration: 'none' }} to={`/reunionPersonalSemanalJefeVentas/rps/siguientes/detalles/${reunion.idReunion}`}>
...
export default RpsTodasJefeVentas
