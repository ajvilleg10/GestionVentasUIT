...
const validationSchema = yup.object({
  ...
  telefono: yup
    .string()
    .required("Campo requerido")
    .test("phone_number_house", "Número de teléfono inválido", (value) => {
      if (value === undefined) return true;
  ...
  fechaNacimiento: yup
    .date("Fecha inválida")
    .required("Campo requerido")
    .test("check-date", "Fecha de nacimiento inválida", (value) => {
  ...
  tituloProfessionOther: yup.string().when("tituloProfession", {
    ...
    then: (schema) =>
      schema
        .matches(/^[^0-9]*$/, "Nombre del título inválido")
        .required("Campo requerido"),
...
const FormularioDeRegistro = () => {
  ...
  const handleCloseModal = () => {
    // Cierra el modal solo si no se ha enviado la información
    if (!isSent) {
      setOpenModal(false);
    }
  };
  ...
  useEffect(() => {
    if (user?.empleadoInfo && user?.cuentaInfo && user?.tipoCuentaInfo) {
      ...
      setInitialValues({
        ...
        detallesExperiencia:
          user?.empleadoInfo?.detalles_experiencia_laboral || "",
      });
...

  {
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        setIsSent(true);
        setIsEditEnabled(true); // Habilitar el botón de editar
  ...
  onSubmit: async (values) => {
    ...
    try {
      ...
      await axios.put(
        `/empleados/updateProspectoVendedorInfo/${user?.empleadoInfo?.id}`,
        newInfo
      );

      // Acciones después de enviar los datos (mensaje de éxito, etc.)
      handleSuccess();
    } catch (error) {
      console.log("form reg", error);
...
const handleConfirm = async () => {
  try {
    ...
    // Lógica específica para el manejo del modal de confirmación
    if (!isSent) {
      // Si no se ha enviado la información, abrir el modal de confirmación
      handleOpenModal();
    } else {
      // Si ya se envió la información, cerrar el modal y realizar acciones necesarias
      handleCloseModal();

      // Enviar los datos solo si aún no se han enviado
      if (!isSent) {
        await handleSubmit();
      }
...
const handleSuccess = () => {
  ...
  // Actualizar el estado como si la información ya se hubiera enviado
  setIsSent(true);
};
...
const handleError = (error) => {
  ...
  const handleEdit = () => {
    // Lógica para habilitar la edición y resetear el formulario si es necesario
    //formik.resetForm();
              ...
              <ListItem divider={!matchDownMD}>
                <Grid container spacing={3}>
                  ...
                  <Grid item xs={12} md={3}>
                    <FormControl fullWidth>
                      <Select
                        ...
                        onChange={(e) => {
                          if (
                            e.target.value ===
                            titulosProfesion[titulosProfesion.length - 1]
                          ) {
                      ...
                      >
                        {titulosProfesion?.map((tituloProfession) => (
                          <MenuItem
                            value={tituloProfession}
                            key={tituloProfession}
                          >
                            {tituloProfession}
                          </MenuItem>
                        ))}
                    ...
                    {Boolean(formik.errors.tituloProfession) &&
                      formik.touched.tituloProfession && (
                        <FormHelperText
                          error
                          id="standard-weight-helper-text-email-login"
                        >
                    ...
                    <Grid item xs={12} md={4}>
                      <FormControl fullWidth>
                        <TextField
                          ...
                          inputProps={{ maxLength: 30 }}
                          error={
                            formik.touched.tituloProfessionOther &&
                            Boolean(formik.errors.tituloProfessionOther)
                          }
                          helperText={
                            formik.touched.tituloProfessionOther &&
                            formik.errors.tituloProfessionOther
                          }
                        />
                      </FormControl>
                      {formik.errors.tituloProfessionOther && (
                  ...
                  <Grid item xs={12} md={3}>
                    <FormControl>
                      <TextField
                        ...
                        onChange={formik.handleChange}
                        disabled={
                          formik.values.estadoCivil === "Soltero" ||
                          formik.values.estadoCivil === "" ||
                          isSent
                        }
                        inputProps={{ maxLength: 30 }}
                        error={
                          formik.touched.conyuge &&
                          Boolean(formik.errors.conyuge)
                        }
                        helperText={
                          formik.touched.conyuge && formik.errors.conyuge
                        }
                      />
                    </FormControl>
                  </Grid>
                ...
                <Grid container spacing={3}>
                  ...
                  <Grid item xs={12} md={3}>
                    <TextField
                      ...
                      disabled={isSent}
                      error={
                        formik.touched.numeroHijos &&
                        Boolean(formik.errors.numeroHijos)
                      }
                      helperText={
                        formik.touched.numeroHijos && formik.errors.numeroHijos
                      }
                    />
                  </Grid>
                </Grid>
        ...
        <Grid item xs={12}>
          <MainCard title="Referencia Laboral">
            <List sx={{ py: 0 }}>
              <ListItem divider={!matchDownMD}>
                <Grid item xs={12} md={3} display="flex" alignItems="center">
                  <InputLabel htmlFor="nombreCompania">
                ...
                <Grid item xs={12} md={4}>
                  <TextField
                    ...
                    disabled={isSent}
                    error={
                      formik.touched.nombreCompania &&
                      Boolean(formik.errors.nombreCompania)
                    }
                    helperText={
                      formik.touched.nombreCompania &&
                      formik.errors.nombreCompania
                    }
                  />
                </Grid>
              </ListItem>
              <ListItem divider={!matchDownMD}>
                <Grid item xs={12} md={3} display="flex" alignItems="center">
                  <InputLabel htmlFor="direccionCompania">
                    Dirección*
                  </InputLabel>
                </Grid>
                <Grid item xs={12} md={4}>
                  <TextField
                    ...
                    disabled={isSent}
                    error={
                      formik.touched.direccionCompania &&
                      Boolean(formik.errors.direccionCompania)
                    }
                    helperText={
                      formik.touched.direccionCompania &&
                      formik.errors.direccionCompania
                    }
                  />
                </Grid>
              </ListItem>
              <ListItem divider={!matchDownMD}>
                ...
                <Grid item xs={12} md={4}>
                  <PatternFormat
                    ...
                    disabled={isSent}
                    error={
                      formik.touched.telefonoCompania &&
                      Boolean(formik.errors.telefonoCompania)
                    }
                    helperText={
                      formik.touched.telefonoCompania &&
                      formik.errors.telefonoCompania
                    }
                  />
                </Grid>
              </ListItem>
              <ListItem divider={!matchDownMD}>
                <Grid item xs={12} md={3} display="flex" alignItems="center">
                  <InputLabel htmlFor="referenciaLaboral">
                    Referencia laboral*
                  </InputLabel>
                </Grid>
                <Grid item xs={12} md={4}>
                  <TextField
                    ...
                    disabled={isSent}
                    error={
                      formik.touched.referenciaLaboral &&
                      Boolean(formik.errors.referenciaLaboral)
                    }
                    helperText={
                      formik.touched.referenciaLaboral &&
                      formik.errors.referenciaLaboral
                    }
                  />
                </Grid>
              </ListItem>
              <ListItem divider={!matchDownMD}>
                <Grid container spacing={3}>
                  <Grid item xs={12} md={3} display="flex" alignItems="center">
                    <InputLabel htmlFor="experienciaEnVentas">
                ...
                <Grid container spacing={3}>
                  <Grid item xs={12} md={12} display="flex" alignItems="center">
                    <InputLabel htmlFor="detallesExperiencia">
                      Favor dar detalles de su ocupación actual o último cargo
                      desempeñado*
                    </InputLabel>
                  </Grid>
                  <Grid item xs={12} md={12}>
                    <TextField
                      ...
                      onChange={formik.handleChange}
                      error={
                        formik.touched.detallesExperiencia &&
                        Boolean(formik.errors.detallesExperiencia)
                      }
                      helperText={
                        formik.touched.detallesExperiencia &&
                        formik.errors.detallesExperiencia
                      }
                    />
                  </Grid>
                </Grid>
