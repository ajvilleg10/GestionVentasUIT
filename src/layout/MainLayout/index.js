...
// import administrador from 'menu-items/CEO';
...
const MainLayout = () => {
  ...
  useEffect(() => {
    if (user?.type === 'Prospecto Vendedor') {
      ...
      setNavigation({ items: [administrador] });
    } else if (user?.type === 'CEO') {
      setNavigation({ items: [CEO] });
    } else if (user?.type === 'Jefe de Sucursal') {
      setNavigation({ items: [jefeSucursal] });
    } else if (user?.type === 'Director de Operaciones') {
      setNavigation({ items: [director] });
    }
